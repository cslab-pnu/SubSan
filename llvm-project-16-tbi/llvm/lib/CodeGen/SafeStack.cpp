...
static cl::opt<bool> ClSubsanAblateStackMeta(
    "subsan-ablate-stack-meta", cl::init(false), cl::Hidden,
    cl::desc("Subsan ablation: skip stack alloca shadow tag set (prologue) and "
             "untag (epilogue) emit. Unsafe-stack relocation remains. "
             "default=false."));

static cl::opt<bool> ClSubsanAblateCheck(
    "subsan-ablate-check", cl::init(false), cl::Hidden,
    cl::desc("Subsan ablation: skip emitting trap (`brk #0x0`) on inline shadow "
             "check mismatch. Shadow load/compare IR still emitted but trap path "
             "elided. default=false."));

static cl::opt<bool> ClSubsanDisableEXO(
    "subsan-disable-exo", cl::init(false), cl::Hidden,
    cl::desc("Subsan opt-ablation: disable EXO (Existing Optimizations — ASan-- "
             "static safety analysis). default=false (= EXO active)."));
static cl::opt<bool> ClSubsanDisableDRP(
    "subsan-disable-drp", cl::init(false), cl::Hidden,
    cl::desc("Subsan opt-ablation: disable DRP (Dominance-based Redundancy "
             "Pruning). default=false."));
static cl::opt<bool> ClSubsanDisableLRH(
    "subsan-disable-lrh", cl::init(false), cl::Hidden,
    cl::desc("Subsan opt-ablation: disable LRH (Loop-scope Range Hoisting). "
             "default=false."));
static cl::opt<bool> ClSubsanDisableCSC(
    "subsan-disable-csc", cl::init(false), cl::Hidden,
    cl::desc("Subsan opt-ablation: disable CSC (Cross-Site Check Coalescing). "
             "default=false."));
...
static void emitInlineTagMemory(IRBuilder<> &IRB, Module *M,
  ...
  llvm::Value *GranuleStart = IRB.CreateAnd(
      Shifted, llvm::ConstantInt::get(I64Ty, 0x000FFFFFFFFFFFFFULL),
  ...
  if (ConstTag == 0) {
    ...
    Meta = IRB.CreateOr(End,
        llvm::ConstantInt::get(I64Ty, uint64_t(ConstTag) << 56),
        "subsan.tagmem.meta");
  }
      ...
      for (GlobalVariable *GV : Globals) {
        ...
        Constant *Aliasee = ConstantExpr::getIntToPtr(
            ConstantExpr::getAdd(
                ...
                ConstantInt::get(I64Ty, uint64_t(Tag) << 56)),
            GV->getType());
...
Value *SafeStack::moveStaticAllocasToUnsafeStack(
  ...
  auto NextStackTag = []() -> uint8_t {
    static uint8_t module_alloca_tag = 1;
    uint8_t t = module_alloca_tag;
    // stack range: 1..85 cycle.
    module_alloca_tag = (module_alloca_tag >= 85) ? 1 : (module_alloca_tag + 1);
    return t;
  };
  ...
  if(!isImplicitTagging) { // alloca tagging is not needed when implicit
    ...
    uint64_t tag_shifted = uint64_t(this_tag) << 56;
...
void SafeStack::InsertCheckRange(Instruction &I, Value *start, Value *end, Type* ptrType) {
  ...
  Value *GranuleIdx = builder.CreateAnd(
      Shifted, ConstantInt::get(Int64Ty, 0x000FFFFFFFFFFFFFULL),
  ...
  Value *MetaPtr = builder.CreateInBoundsGEP(Int64Ty, ShadowI64Base, GranuleIdx,
  ...
  Value *Masked = builder.CreateAnd(
      Diff, ConstantInt::get(Int64Ty, 0xFFFF000000000000ULL),
      "subsan.range.masked");
...
bool SafeStack::ChecksOnFunc(Function &F, ObjectSizeOffsetVisitor &ObjSizeVis) {
  if (ClSubsanAblateCheck) return true;
  ...
  // === DRP — Dominance-based Redundancy Pruning ===
  if (!ClSubsanDisableDRP) {
  ...
  // === LRH transform — BaseBounds loop hoisting ===
  if (!ClSubsanDisableLRH) {
    LoopOptimization(F, OperandsToInstrument, invariants, CheckMergers, FuncMetaMergers, &LI, &SE);
  }

  // === CSC transform — block/pair merging + metadata sharing ===
  if (!ClSubsanDisableCSC) {
  ...
  // === Final: emit check IR for all remaining operands ===
